"""Download an immutable Met Office Wales mean-temperature source snapshot."""
...
def download_source(output_dir: Path, *, url: str = SERIES_URL) -> DownloadResult:
    """Fetch the exact HTTP response bytes and preserve them without transformation."""
    request = Request(url, headers={"User-Agent": "Hinsawdd-Cymru/0.2 (+public reproducible research)"})
    ...
    if "year" not in text or "ann" not in text:
        raise ValueError("Official source header is incomplete")
    ...
    stem = f"metoffice-wales-tmean-retrieved-{retrieval_token}"
