"""Download an immutable Met Office Wales rainfall source snapshot."""
...
def download_source(output_dir: Path, *, url: str = SERIES_URL) -> DownloadResult:
    ...
    request = Request(url, headers={"User-Agent": "Hinsawdd-Cymru/0.3 (+public reproducible research)"})
    ...
    if any(item not in text for item in required):
        raise ValueError("Official rainfall source header is incomplete")
    ...
    stem = f"metoffice-wales-rainfall-retrieved-{retrieval_token}"
