...
def annual_reconciliation(bundle: SourceBundle) -> pd.DataFrame:
    monthly = bundle.monthly.groupby("year", as_index=False).agg(
    ...
    result = monthly.merge(official, on="year", how="left")
...
def _rolling_periods(monthly: pd.DataFrame, months: int, ending_month: int) -> pd.DataFrame:
    ...
    selected = totals[totals.index.month == ending_month]
...
def fit_linear(years: pd.Series | np.ndarray, values: pd.Series | np.ndarray) -> LinearFit:
    ...
    if len(x_years) < 3 or len(x_years) != len(y):
        raise ValueError("A linear fit requires at least three paired observations")
    x = x_years - 2000.0
    x_mean = float(x.mean())
    y_mean = float(y.mean())
    ...
    residual_se = math.sqrt(ss_res / (len(y) - 2))
    return LinearFit(
        intercept_at_2000_mm=intercept,
        slope_mm_per_year=slope,
        r_squared=r_squared,
        residual_standard_error_mm=residual_se,
        observation_count=len(y),
        first_end_year=int(x_years.min()),
        last_end_year=int(x_years.max()),
    )
...
def fit_theil_sen(years: pd.Series | np.ndarray, values: pd.Series | np.ndarray) -> LinearFit:
    ...
    intercepts = y - slope * (x_years - 2000.0)
    intercept = float(np.median(intercepts))
    fitted = intercept + slope * (x_years - 2000.0)
    residuals = y - fitted
    ss_res = float(np.sum(residuals**2))
    ss_tot = float(np.sum((y - y.mean()) ** 2))
    r_squared = 1.0 - ss_res / ss_tot if ss_tot else 0.0
    residual_se = math.sqrt(ss_res / max(len(y) - 2, 1))
    return LinearFit(
        intercept_at_2000_mm=intercept,
        slope_mm_per_year=slope,
        r_squared=r_squared,
...
def bootstrap_projection(
    ...
    for replicate in range(config.bootstrap_replicates):
        ...
        boot_values = fitted + np.asarray(sample[:n])
        boot_fit = fit_linear(x_years, boot_values)
        predictions[replicate] = boot_fit.predict(years)

    return (
        np.quantile(predictions, 0.025, axis=0),
        np.quantile(predictions, 0.975, axis=0),
    )
...
def run_backtests(series: pd.DataFrame, config: AnalysisConfig) -> pd.DataFrame:
    ...
    for cutoff in config.backtest_cutoffs:
        ...
        if len(training) < 10 or len(actual) != config.backtest_horizon_years:
            continue
        fit = fit_linear(training["end_year"], training["rainfall_total_mm"])
        predictions = fit.predict(actual["end_year"])
...
def seasonal_trends(annual: pd.DataFrame, reference: float, modern_start: int) -> pd.DataFrame:
    ...
    for column in SEASON_COLUMNS:
        ...
        modern = fit_linear(
            subset.loc[subset["year"] >= modern_start, "year"],
            subset.loc[subset["year"] >= modern_start, "rainfall_mm"],
        )
        rows.append({
...
def make_history_figure(
    ...
    for row, label, offset in (
        (wettest, "Wettest", (12, 16)),
        (driest, "Driest", (12, -32)),
        (latest, "Latest complete", (-12, 16)),
    ):
    ...
    fig.text(
        ...
        0.012,
        (
            "Data source: Met Office National Climate Information Centre, Wales HadUK-Grid 1 km areal rainfall series. "
...
def run(
    ...
    *,
    ...
    latest_complete = complete.iloc[-1]
    current_partial = partials.iloc[-1]
    ...
    wettest = complete.nlargest(10, "rainfall_total_mm").copy()
    driest = complete.nsmallest(10, "rainfall_total_mm").copy()
    wet_rows = [
        {
            "rank": index,
            "period": row.period,
        ...
        for index, row in enumerate(wettest.itertuples(), start=1)
    ]
    dry_rows = [
        {
            "rank": index,
        ...
        for index, row in enumerate(driest.itertuples(), start=1)
    ]

    milestone_rows: dict[str, dict[str, float]] = {}
    for year in (2050, 2100):
        row = projection.loc[projection["end_year"] == year].iloc[0]
        milestone_rows[str(year)] = {
...
def main() -> None:
    ...
    summary = run(args.source, update_project_readme=not args.no_update_readme)
